...
// the transpiler/runtime-template source of truth) and displays each composited
// frame via PixiJS.
//
...
void (async () => {
  ...
  try {
    ...
    function dispatchBehaviorEvent(
      ...
      console.log(`[behavior] frame=${frameIndex + 1} channel=${bc.channel} event=${event} script=${bc.scriptName}`);
    ...
    function dispatchMovieScriptEvent(event: string, ...args: unknown[]): void {
      for (const mod of loadedScripts) {
        ...
        for (const handler of mod.handlers) {
          console.log(`[moviescript] frame=${frameIndex + 1} event=${event} script=${mod.scriptName} handler=${handler.name}`);
          if (handler.event === event) {
            const stub = mod.handlerStubs[handler.name];
            if (!stub) {
              continue;
    ...
    function dispatchFrameScriptEvent(
      ...
      console.log(`[framescript] frame=${frameIndex + 1} event=${event} script=${fs.scriptName}`);
    ...
    // Live frame state that survives across ticks so Lingo mutations (e.g. random stars
    // setting locH/locV) persist until the score explicitly overrides them again. The
    // exporter's score.json gives the static per-frame state; we merge only the properties
    // that actually changed in the score, leaving script-driven changes intact.
    let liveSnapshot: runtime.FrameSnapshot | null = null;
    let lastScoreSnapshot: runtime.FrameSnapshot | null = null;

    function cloneSprite(s: runtime.RenderSprite): runtime.RenderSprite {
      return { ...s };
    }

    function cloneSnapshot(s: runtime.FrameSnapshot): runtime.FrameSnapshot {
      return { ...s, sprites: s.sprites.map(cloneSprite) };
    }

    function hydrateFrame(idx: number): void {
      const scoreSnap = buildFrameSnapshot(score, cast, idx, loadBitmap, bakeTick);
      if (!liveSnapshot || !lastScoreSnapshot) {
        liveSnapshot = cloneSnapshot(scoreSnap);
      } else {
        const scoreByCh = new Map(scoreSnap.sprites.map((s) => [s.channel, s]));
        const lastByCh = new Map(lastScoreSnapshot.sprites.map((s) => [s.channel, s]));
        const liveByCh = new Map(liveSnapshot.sprites.map((s) => [s.channel, s]));
        const nextSprites: runtime.RenderSprite[] = [];

        for (const s of scoreSnap.sprites) {
          const last = lastByCh.get(s.channel);
          const live = liveByCh.get(s.channel);
          if (!live) {
            nextSprites.push(cloneSprite(s));
            continue;
          }
          const next: runtime.RenderSprite = { ...live };
          if (!last) {
            Object.assign(next, s);
          } else {
            if (s.x !== last.x) next.x = s.x;
            if (s.y !== last.y) next.y = s.y;
            if (s.width !== last.width) next.width = s.width;
            if (s.height !== last.height) next.height = s.height;
            if (s.locZ !== last.locZ) next.locZ = s.locZ;
            if (s.visible !== last.visible) next.visible = s.visible;
            if (s.type !== last.type) next.type = s.type;
            if (s.foreColor !== last.foreColor) next.foreColor = s.foreColor;
            if (s.backColor !== last.backColor) next.backColor = s.backColor;
            if (s.hasForeColor !== last.hasForeColor) next.hasForeColor = s.hasForeColor;
            if (s.hasBackColor !== last.hasBackColor) next.hasBackColor = s.hasBackColor;
            if (s.ink !== last.ink) next.ink = s.ink;
            if (s.blend !== last.blend) next.blend = s.blend;
            if (s.flipH !== last.flipH) next.flipH = s.flipH;
            if (s.flipV !== last.flipV) next.flipV = s.flipV;
            if (s.rotation !== last.rotation) next.rotation = s.rotation;
            if (s.skew !== last.skew) next.skew = s.skew;
            if (s.bakedBitmap !== last.bakedBitmap) next.bakedBitmap = s.bakedBitmap;
            if (s.hasBehaviors !== last.hasBehaviors) next.hasBehaviors = s.hasBehaviors;
            if (s.shapeLineSize !== last.shapeLineSize) next.shapeLineSize = s.shapeLineSize;
            if (s.shapePattern !== last.shapePattern) next.shapePattern = s.shapePattern;
          }
          nextSprites.push(next);
        }

        liveSnapshot = {
          frameNumber: scoreSnap.frameNumber,
          stageWidth: scoreSnap.stageWidth,
          stageHeight: scoreSnap.stageHeight,
          backgroundColor: scoreSnap.backgroundColor,
          sprites: nextSprites,
          debugInfo: scoreSnap.debugInfo,
          stageImage: scoreSnap.stageImage,
          bakeTick: scoreSnap.bakeTick,
        };
      }
      lastScoreSnapshot = cloneSnapshot(scoreSnap);
      lingoHost.snapshot = liveSnapshot;
    }
    ...
    function tick(): void {
      // Start of frame: merge the score state for this frame into the live snapshot.
      // Only score-driven changes overwrite Lingo mutations; unchanged properties keep
      // their script-modified values from previous ticks.
      hydrateFrame(frameIndex);
      ...
      // advanceFrame: exitFrame global, then transition to the next frame.
    ...
    // Start paused when the parity harness requests deterministic stepping via URL parameter,
    // so the rAF loop never ticks before the harness owns playback.
    const startPaused = new URLSearchParams(window.location.search).has("__lsPaused");
    ...
    (window as unknown as {
      ...
      __lsBakeTick?: () => number;
      __lsSetPaused?: (paused: boolean) => void;
      __lsSetBakeTick?: (tick: number) => void;
    }).__lsSetPaused = (p) => {
      paused = p;
      if (statusEl) {
    ...
    (window as unknown as {
      ...
      const snapshot = lingoHost.snapshot;
      if (!snapshot) return [];
      return snapshot.sprites.map((s) => ({
        channel: s.channel,
        x: s.x,
        y: s.y,
        width: s.width,
